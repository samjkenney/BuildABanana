...
export class ImageButton extends Button{
    ...
    constructor(scene: Scene, x: number, y: number, width: number, height: number, color: number, imageKey: string, scaleOnHover: boolean, staySelected: boolean, action: Function){
        var content = new GameObjects.Image(scene, 0, 0, imageKey);
        super(scene, x, y, width, height, color, content, true, scaleOnHover, staySelected, action);
